    ...
    useEffect(() => {
        ...
        const loadAnneesUniv = async () => {
            try {
                ...
                const currentYear = "2024-2025"; // Année universitaire actuelle

                const sortedData = data.map(annee => ({
                    id_anneuniv: annee.id_anneuniv,
                    annee: annee.dateuniv,
                    statutuniv: determineStatut(annee.dateuniv, currentYear)
                }));

                setAnneesUniv(sortedData.sort((a, b) => b.annee.localeCompare(a.annee)));
            } catch (error) {
        ...
        // Fonction pour déterminer le statut d'une année universitaire
        const determineStatut = (annee, currentYear) => {
            if (annee === currentYear) return 'en cours';
            if (annee < currentYear) return 'terminé';
            return 'à venir';
        };

        // Charger les promotions via le service
        const loadPromotions = async () => {
            try {
                const data = await PromotionService.getAllPromotions();
                const sortedData = data.map(promotion => ({
                    id_prom: promotion.id_promotion,
                    name_prom: promotion.name_promotion,
                    annee_prom: promotion.annee_promotion.toString()
